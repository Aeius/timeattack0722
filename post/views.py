...
class ApplyView(APIView):
    ...
    def get(self, request):
        param_status = request.GET.get('status', None)
        status_id = JobPostStatus.objects.get(status=param_status).id
        instance = JobPostActivity.objects.filter(status=status_id)
        serializer = JobPostActivitySerializer(instance, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)

    def post(self, request):
        request.data['user'] = request.user.id
        request.data["status"] = JobPostStatus.objects.get(status="submitted").id
